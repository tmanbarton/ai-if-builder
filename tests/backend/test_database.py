...
from backend.database import init_db, insert_file, fetch_file_content
...
def test_insert_and_fetch_roundtrip(test_db):
    ...
    result = fetch_file_content("session-1", "foo.txt", test_db)
...
def test_fetch_missing_returns_none(test_db):
    result = fetch_file_content("nonexistent-session", "nonexistent.txt", test_db)
